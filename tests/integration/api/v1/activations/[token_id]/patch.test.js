  ...
  describe("Anonymous user", () => {
    test("With nonexistent token", async () => {
      const response = await fetch(
        "http://localhost:3000/api/v1/activations/86db9fda-41db-4024-b771-9d21bd7a3cb1",
        {
          method: "PATCH",
        },
      );
    ...
    test("With valid token", async () => {
      ...
      const activatedUset = await user.findOneById(createdUser.id);

      expect(activatedUset.features).toEqual([
        "create:session",
        "read:session",
        "update:user",
      ]);
